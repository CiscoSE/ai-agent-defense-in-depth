...
const UPSTREAM_REPO = process.env.UPSTREAM_REPO || 'your-org/your-repo';
const FORK_OWNER = process.env.FORK_OWNER || 'your-fork-org';
const ENV_FILE = (process.env.AGENT_HOME || '/home/aetherclaude') + '/.env';
const APP_KEY_FILE = (process.env.AGENT_HOME || '/home/aetherclaude') + '/.github-app-key.pem';
const AUDIT_LOG = (process.env.AGENT_HOME || '/home/aetherclaude') + '/logs/mcp-audit.log';
const PROXY = process.env.HTTPS_PROXY || 'http://127.0.0.1:8888';
...
function audit(op, args, result) {
    ...
    try { fs.appendFileSync(AUDIT_LOG, JSON.stringify({timestamp:new Date().toISOString(),operation:op,args:typeof args==='object'?Object.keys(args):[],result:safe})+'\n'); } catch{}
}
...
async function getAppToken() {
    ...
    const tok = await ghAPI('POST',`/app/installations/${installs[0].id}/access_tokens`,null,jwt,true);
    ...
    return new Promise((resolve, reject) => {
        ...
        cr.on('connect',(res,sock) => {
            ...
            const r = https.request({host:'api.github.com',socket:sock,rejectUnauthorized:true,servername:'api.github.com',method,path,headers:hdrs},(resp) => {
                let d=''; resp.on('data',c=>d+=c); resp.on('end',()=>{try{resolve(JSON.parse(d))}catch{resolve(d)}});
            });
            r.on('error',reject);
