...
MAX_EVENTS = 5000
EVENTS_DB = os.environ.get('AGENT_DATA_DIR', '/var/lib/aetherclaude') + '/events.db'
REFRESH_INTERVAL_MS = 3000
TOKEN_REFRESH_SECS = 30
RING_REFRESH_SECS = 15
SESSION_DIR = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/.claude/projects'
VALIDATION_LOG = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/validation.log'
MCP_AUDIT_LOG = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/mcp-audit.log'
ORCHESTRATOR_LOG = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/orchestrator.log'
SESSION_DIR = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/.claude/projects'
...
events_by_source = defaultdict(lambda: deque(maxlen=1000))
...
_SECRET_PATTERNS = [
    ...
    (_re.compile(r'(?:/home/aetherclaude/)?\.gh-token'), '***'),
    (_re.compile(r'(?:/home/aetherclaude/)?\.git-credentials'), '***'),
...
def append_event(entry):
    """Append to global deque + per-source buffer + DB queue. Redacts secrets."""
    entry = dict(entry)  # copy to avoid mutating caller's dict
    entry['args'] = _redact(entry.get('args', ''))
    events.append(entry)
    events_by_source[entry.get('source', 'unknown')].append(entry)
    with db_write_lock:
...
def db_purge_if_needed(max_gb=250):
    ...
    try:
        ...
        if size_gb >= max_gb:
            ...
            try:
                import datetime
                with open(os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/orchestrator.log', 'a') as f:
...
def scan_rings():
    ...
    while True:
        try:
            with lock:
                # Ring 1: nftables
                out = sh("nft list chain inet filter output 2>/dev/null | grep 'counter packets'")
                if out:
                    import re
                    m = re.search(r'packets (\d+) bytes (\d+)', out)
                    if m: ring_stats['r1_packets_blocked'] = int(m.group(1))

                # Ring 2: tinyproxy — r2_allowed and r2_denied are updated by tail_tinyproxy_log

                # Ring 3: OS isolation — count agent processes vs total
                try:
                    total_procs = int(sh("ps -e --no-headers | wc -l") or 0)
                    agent_procs = int(sh(f"ps -u {AETHERCLAUDE_UID} --no-headers 2>/dev/null | wc -l") or 0)
                ...
                try:
                    runs = sh("journalctl -u aetherclaude.service --no-pager 2>/dev/null | grep -c 'Starting AetherClaude'")
                    ring_stats['r4_sandboxed_runs'] = int(runs) if runs.strip().isdigit() else 0
                    # Timer uptime
                    since = sh("systemctl show aetherclaude.timer --property=ActiveEnterTimestamp --value 2>/dev/null")
                ...
                try:
                    if os.path.exists(MCP_SCAN_FILE):
                        ...
                        for detail in mcp_details:
                            ...
                            if not any(e.get('source') == 'mcp-scan' and e.get('args','').startswith(detail['name']) for e in list(events)[-50:]):
                                append_event(entry)
                except: pass

                try:
                    # Scan the actual skills directory with skill-scanner
                    skills_dir = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/skills'
                    skill_results = []
                    total_findings = 0
                    if os.path.isdir(skills_dir):
                        ...
                        skills_scanned = len(skill_results) if skill_results else len([f for f in os.listdir(skills_dir) if f.endswith('.md')])
                        ...
                        if skill_results and not any(e.get('source') == 'skill-scan' for e in list(events)[-20:]):
                            try:
                                dbc = sqlite3.connect(EVENTS_DB)
                    ...
                    workspace_cmds = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/workspace/' + os.environ.get('PROJECT_NAME', 'project') + '/.claude/commands'
                    injected_files = []
                    ...
                    if not any(e.get('source') == 'skill-scan' for e in list(events)[-20:]):
                        append_event(entry)
                except: ring_stats['r6_skill_status'] = 'error'

                # AIBOM: read C++ AI Bill of Materials
                try:
                    aibom_file = os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/aibom-latest.json'
                ...
                try:
                    with open(os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/.claude/settings.json') as f:
                        s = json.load(f)
                ...
                try:
                    ...
                    token = sh(os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/bin/github-app-token.sh')
                    if token:
                        proxy_handler = urllib.request.ProxyHandler({'https': 'http://127.0.0.1:8888'})
                        opener = urllib.request.build_opener(proxy_handler)
                        req = urllib.request.Request(
                            f'https://api.github.com/repos/{os.environ.get("UPSTREAM_REPO", "your-org/your-repo")}/pulls?state=all&per_page=100',
                            headers={'Authorization': f'token {token}', 'Accept': 'application/vnd.github+json', 'User-Agent': 'AetherClaude-Dashboard'})
                        out = opener.open(req, timeout=10).read().decode()
                    else:
                        out = ''
                except Exception:
                    out = ''
                if out:
                    try:
                        prs = json.loads(out)
                        ac = [p for p in prs if p.get('user', {}).get('login') in (os.environ.get('FORK_OWNER', 'your-fork-org'), os.environ.get('BOT_USERNAME', 'your-app[bot]'))]
                        ring_stats['r9_prs_total'] = len(ac)
                        ring_stats['r9_prs_merged'] = len([p for p in ac if p.get('merged_at')])
                        ring_stats['r9_prs_rejected'] = len([p for p in ac if p['state'] == 'closed' and not p.get('merged_at')])
                        ring_stats['r9_prs_open'] = len([p for p in ac if p['state'] == 'open'])
                        ring_stats['r9_pr_details'] = {
                            'open': [{'number': p['number'], 'title': p['title'], 'draft': p.get('draft', False)} for p in ac if p['state'] == 'open'],
                            'merged': [{'number': p['number'], 'title': p['title']} for p in ac if p.get('merged_at')][:20],
                            'rejected': [{'number': p['number'], 'title': p['title']} for p in ac if p['state'] == 'closed' and not p.get('merged_at')][:20],
                        }
                    except: pass

                # Fetch recent issues commented on by agent
                try:
                    if token:
                        req_i = urllib.request.Request(
                            f'https://api.github.com/repos/{os.environ.get("UPSTREAM_REPO", "your-org/your-repo")}/issues?state=all&per_page=30&sort=updated&direction=desc',
                            headers={'Authorization': f'token {token}', 'Accept': 'application/vnd.github+json', 'User-Agent': 'AetherClaude-Dashboard'})
                        issues_out = opener.open(req_i, timeout=10).read().decode()
                        issues = json.loads(issues_out)
                        # Issues that agent has interacted with (exclude PRs)
                        agent_issues = [{'number': i['number'], 'title': i['title'], 'state': i['state']}
                            for i in issues if not i.get('pull_request') and
                            any(l.get('name','') in ('claude-active','awaiting-confirmation','aetherclaude-eligible') for l in i.get('labels',[]))]
                        ring_stats['r9_issue_details'] = agent_issues[:20]
                except: pass

                # Fetch recent discussions
                try:
                    if token:
                        gql_body = json.dumps({'query': 'query{repository(owner:"' + os.environ.get('UPSTREAM_OWNER', 'your-org') + '",name:"' + os.environ.get('PROJECT_NAME', 'your-repo') + '"){discussions(first:20,orderBy:{field:UPDATED_AT,direction:DESC}){nodes{number title category{name} comments{totalCount}}}}}'}).encode()
...
def tail_tinyproxy_log():
    """Watch tinyproxy journal for proxy connections and denials."""
    import subprocess, re
    proc = subprocess.Popen(
        ['journalctl', '-u', 'tinyproxy', '-f', '--no-pager', '-o', 'short-iso'],
    ...
    for line in proc.stdout:
        ...
        with lock:
            ...
            if 'Proxying refused on filtered domain' in line:
                ...
                append_event(entry)
                stats['alerts'].append({'time': ts, 'msg': f"PROXY: Blocked domain {domain}", 'severity': 'high'})
...
def tail_nftables_log():
    """Watch kernel log for nftables blocked packets from aetherclaude."""
    import subprocess
    proc = subprocess.Popen(
        ['journalctl', '-k', '-f', '-g', 'AETHERCLAUDE_BLOCKED', '--no-pager', '-o', 'short-iso'],
        stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True
    )
    for line in proc.stdout:
        line = line.strip()
        if 'AETHERCLAUDE_BLOCKED' not in line:
            continue
        with lock:
            ...
            detail = f"{proto} → {dst}:{dpt}" if dst else line[:80]
            ...
            entry = {
                ...
                'uid': 965,
                'binary': 'nftables',
                'args': f"Outbound blocked: {detail}",
                'policy': 'nftables',
                'is_agent': True,
                'source': 'nftables'
            }
...
<label>Show:</label>
<button class="fbtn" onclick="tf(this,'all')" id="f-all">All</button>
<button class="fbtn active" onclick="tf(this,'agent')" id="f-agent">AetherClaude</button>
<button class="fbtn" onclick="tf(this,'exec')">EXEC</button>
<button class="fbtn" onclick="tf(this,'kprobe')">KPROBE</button>
<button class="fbtn" onclick="tf(this,'exit')">EXIT</button>
<button class="fbtn active" onclick="tf(this,'block')" style="border-color:#ff4444">FIREWALL</button>
<button class="fbtn active" onclick="tf(this,'proxy')" style="border-color:#44ddaa">PROXY</button>
<button class="fbtn active guard" onclick="tf(this,'guard')">CODEGUARD</button>
<button class="fbtn active mcp-f" onclick="tf(this,'mcp')">MCP</button>
<button class="fbtn active" onclick="tf(this,'scan')" style="border-color:#00ddff">SCANNERS</button>
<button class="fbtn active" onclick="tf(this,'tool')" style="border-color:#ff88cc">CLAUDE</button>
<button class="fbtn active" onclick="tf(this,'skill')" style="border-color:#ffdd44">SKILLS</button>
<label>|</label>
<button class="fbtn active" onclick="tf(this,'nosys')">Hide System</button>
<label>Search:</label>
<input class="finput" id="search" placeholder="filter..." oninput="af()">
...
<script>
let fl={all:0,agent:1,exec:0,kprobe:0,exit:0,block:1,proxy:1,guard:1,mcp:1,scan:1,tool:1,skill:1,nosys:1},ae=[];
const SYS=['sshd','sshd-session','sshd-auth','unix_chkpwd','systemd','systemd-user-runtime-dir','systemd-xdg-autostart-generator','30-systemd-environment-d-generator','9'];
function tf(b,k){if(k==='all'){fl.all=1;fl.agent=0;document.getElementById('f-all').classList.add('active');document.getElementById('f-agent').classList.remove('active')}else if(k==='agent'){fl.agent=1;fl.all=0;document.getElementById('f-agent').classList.add('active');document.getElementById('f-all').classList.remove('active')}else{fl[k]=fl[k]?0:1;b.classList.toggle('active')}af()}
function af(){
// If only one source type is active, show its full 1000-event buffer
const search=document.getElementById('search').value.toLowerCase();
const activeTypes=[];
if(fl.exec||fl.kprobe||fl.exit) activeTypes.push('tetragon');
if(fl.block) activeTypes.push('nftables');
if(fl.proxy) activeTypes.push('tinyproxy');
if(fl.guard) activeTypes.push('codeguard');
if(fl.mcp) activeTypes.push('mcp');
if(fl.scan) activeTypes.push('mcp-scan','skill-scan');
if(fl.tool) activeTypes.push('claude-code');
if(fl.skill) activeTypes.push('skill-dispatch');

// If filtering to specific sources, fetch their dedicated buffers
if(!fl.all && activeTypes.length>0 && activeTypes.length<=3){
const srcEvents=[];
for(const src of activeTypes){
const buf=lastData.source_buffers?.[src]||[];
srcEvents.push(...buf);
}
if(srcEvents.length>0){ae=srcEvents.sort((a,b)=>(b.time||'').localeCompare(a.time||''));}
}const s=document.getElementById('search').value.toLowerCase();let h='',n=0;
for(const e of ae){if(!fl[e.type.toLowerCase()]&&e.type)continue;if(fl.agent&&!e.is_agent)continue;
if(fl.nosys&&!e.is_agent){const b=e.binary.split('/').pop();if(SYS.includes(b))continue}
if(s){const x=(e.binary+' '+e.args+' '+e.policy).toLowerCase();if(!x.includes(s))continue}
n++;let t='';
if(e.time){
try{const dt=new Date(e.time);t=dt.toLocaleTimeString('en-US',{hour12:false,hour:'2-digit',minute:'2-digit',second:'2-digit'})}
catch{t=e.time.substring(11,19)}
}
...
document.getElementById('evts').innerHTML=h;document.getElementById('sc').textContent=`${n}/${ae.length}`}
function esc(s){return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}
function refresh(){fetch('/api/events').then(r=>r.json()).then(d=>{
...
document.getElementById('r3v').textContent=r.r3_agent_cmds||0;
document.getElementById('r3d').textContent=`agent cmds · ${r.r3_agent_procs||0} active procs`;
...
else ph+=`<div class="si"><span class="n"><span style="color:#505060;font-size:10px">${t}</span> <span style="color:${col}">${label}</span> #${a.num}</span></div>`;
}
...
storeData(d);ae=[...d.events].reverse();af()}).catch(()=>{})}
...
h+=`<div class="modal-finding ${r.r6_skill_findings>0?'MEDIUM':'SAFE'}"><span class="sev ${r.r6_skill_findings>0?'MEDIUM':'SAFE'}">${r.r6_skill_findings||0}</span> findings across ${r.r6_skills_scanned||'?'} skills in agent skills/</div>`;
...
const buf=lastData.source_buffers||{};
const evts=buf[source]||[];
let h=`<p style="color:#607080;margin-bottom:12px">${desc}</p>`;
h+=`<div class="modal-finding SAFE"><span class="sev SAFE">EVENTS</span> ${evts.length} events in memory buffer</div>`;
if(evts.length===0){h+='<p style="color:#607080;margin-top:12px">No events captured yet.</p>';}
else{
h+='<div style="margin-top:12px;max-height:50vh;overflow-y:auto;font-family:monospace;font-size:11px">';
...
h+=`<div class="modal-finding ${cls}" style="margin-bottom:2px;padding:4px 8px">`;
h+=`<span style="color:#607080;margin-right:8px">${esc(e.time||'').substring(11,19)}</span>`;
if(e.type)h+=`<span style="color:#ffaa00;margin-right:6px;font-weight:bold">${esc(e.type)}</span>`;
h+=`${esc(e.args||'')}`;
if(e.policy)h+=` <span style="color:#ff6688;font-size:10px">[${esc(e.policy)}]</span>`;
h+=`</div>`}
if(sorted.length>200)h+=`<p style="color:#607080;margin-top:8px">Showing 200 of ${sorted.length} events</p>`;
h+='</div>'}
...
for(const p of d.open)fh+=`<div class="modal-finding MEDIUM" style="margin-bottom:3px;padding:4px 8px"><a href="https://github.com/'+encodeURI(lastData.rings?.upstream_repo||'your-org/your-repo')+'/pull/${p.number}" target="_blank" style="color:#00b4d8;text-decoration:none">#${p.number}</a> ${esc(p.title)}${p.draft?' <span style="color:#607080;font-size:10px">(draft)</span>':''}</div>`}
...
for(const p of d.merged)fh+=`<div class="modal-finding SAFE" style="margin-bottom:3px;padding:4px 8px"><a href="https://github.com/'+encodeURI(lastData.rings?.upstream_repo||'your-org/your-repo')+'/pull/${p.number}" target="_blank" style="color:#00b4d8;text-decoration:none">#${p.number}</a> ${esc(p.title)}</div>`}
if(d.rejected&&d.rejected.length>0){
fh+='<p style="color:#607080;margin-bottom:6px;margin-top:12px;font-weight:bold">Rejected:</p>';
for(const p of d.rejected)fh+=`<div class="modal-finding HIGH" style="margin-bottom:3px;padding:4px 8px"><a href="https://github.com/'+encodeURI(lastData.rings?.upstream_repo||'your-org/your-repo')+'/pull/${p.number}" target="_blank" style="color:#00b4d8;text-decoration:none">#${p.number}</a> ${esc(p.title)}</div>`}
if(d.issues&&d.issues.length>0){
fh+='<p style="color:#607080;margin-bottom:6px;margin-top:12px;font-weight:bold">Issues triaged by agent:</p>';
for(const i of d.issues)fh+=`<div class="modal-finding ${i.state==='open'?'MEDIUM':'SAFE'}" style="margin-bottom:3px;padding:4px 8px"><a href="https://github.com/'+encodeURI(lastData.rings?.upstream_repo||'your-org/your-repo')+'/issues/${i.number}" target="_blank" style="color:#00b4d8;text-decoration:none">#${i.number}</a> ${esc(i.title)} <span style="color:#607080;font-size:10px">(${i.state})</span></div>`}
...
for(const disc of d.discussions)fh+=`<div class="modal-finding SAFE" style="margin-bottom:3px;padding:4px 8px"><a href="https://github.com/'+encodeURI(lastData.rings?.upstream_repo||'your-org/your-repo')+'/discussions/${disc.number}" target="_blank" style="color:#00b4d8;text-decoration:none">#${disc.number}</a> ${esc(disc.title)} <span style="color:#607080;font-size:10px">${esc(disc.category)} · ${disc.comments} comments</span></div>`}
...
class H(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path=='/':
            ...
            try:
                with open(os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/agent-sbom.cdx.json', 'r') as sf:
                    data = sf.read().encode()
            ...
            try:
                with open(os.environ.get('AGENT_HOME', '/home/aetherclaude') + '/logs/sbom.cdx.json', 'r') as sf:
                    data = sf.read().encode()
                self.send_response(200)
                self.send_header('Content-Type', 'application/json')
...
{wp_html}</body></html>"""
            ...
            try:
                with open(os.environ.get('DASHBOARD_ASSETS', '/usr/local/share/tetragon-dashboard') + '/logo.png', 'rb') as lf:
                    data = lf.read()
                self.send_response(200)
    ...
    def do_POST(self):
        if self.path == '/webhook':
            ...
            if sender in (os.environ.get('FORK_OWNER', 'your-fork-org'), os.environ.get('BOT_USERNAME', 'your-app[bot]'), os.environ.get('UPSTREAM_OWNER', 'your-org')):
            ...
            # Trigger agent run
            try:
                subprocess.Popen(['sudo', 'systemctl', 'start', 'aetherclaude.service'],
