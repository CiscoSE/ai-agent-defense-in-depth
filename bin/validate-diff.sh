...
WORKSPACE="${1:-.}"
LOGFILE="${AGENT_HOME:-/home/aetherclaude}/logs/validation.log"

cd "$WORKSPACE"

log() {
    echo "$(date -Iseconds) VALIDATE: $1" >> "$LOGFILE"
}
...
echo "[]" > ${AGENT_HOME:-/home/aetherclaude}/logs/codeguard-latest.json 2>/dev/null || true
...
for file in $CHANGED_FILES; do
    if [[ "$file" != src/* ]] && [[ "$file" != docs/* ]] && [[ "$file" != CMakeLists.txt ]] && [[ "$file" != third_party/* ]] && [[ "$file" != packaging/* ]]; then
...
for pattern in "${SUSPICIOUS_PATTERNS[@]}"; do
    MATCHES=$(echo "$DIFF_CONTENT" | grep -cP "^\+.*$pattern" 2>/dev/null || echo 0)
...
CODEGUARD="${AGENT_HOME:-/home/aetherclaude}/defenseclaw/defenseclaw-gateway"
if [ -x "$CODEGUARD" ]; then
...
try:
    ...
    if findings:
        ...
        existing = []
        try:
            with open('${AGENT_HOME:-/home/aetherclaude}/logs/codeguard-latest.json') as f:
                existing = json.load(f)
        except: pass
        existing.extend(findings)
        with open('${AGENT_HOME:-/home/aetherclaude}/logs/codeguard-latest.json', 'w') as f:
            json.dump(existing, f)
        # Insert into SQLite
        db = sqlite3.connect('${AGENT_DATA_DIR:-/var/lib/aetherclaude}/events.db')
